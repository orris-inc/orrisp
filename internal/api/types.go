...
type Outbound struct {
	Type   string `json:"type"`        // Protocol type: shadowsocks, trojan, vless, vmess, hysteria2, tuic, anytls, direct, block
...
// IsAnyTLS returns true if the node is configured for AnyTLS protocol.
func (c *NodeConfig) IsAnyTLS() bool {
	return c.Protocol == "anytls"
}
...
// Used as the Extra payload when sending EventTypeTraffic events via WebSocket.
//
// Usage:
//
//	reports := []agent.TrafficReport{
//	    {SubscriptionSID: "sub_xxx", Upload: 1024, Download: 2048},
//	}
//	hubClient.SendEvent(agent.EventTypeTraffic, "", reports)
type TrafficReport struct {
	SubscriptionSID string `json:"subscription_id"` // Subscription SID (Stripe-style: sub_xxx)
	Upload          int64  `json:"upload"`          // Upload bytes since last report
	Download        int64  `json:"download"`        // Download bytes since last report
}
